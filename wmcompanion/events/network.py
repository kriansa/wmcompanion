...
from ..utils.dbus_client import SystemDBusClient, DBusClientError
...
class WifiStatus(EventListener):
    ...
    dbus: SystemDBusClient = None
    wifi_enabled: bool = False
    wifi_connected: bool = False
    wifi_strength: str = ""
    wifi_access_point_path: str = ""

    async def update_state(self):
        """
        Triggers a state update with all Wifi details
        """
    ...
    async def start(self):
        ...
        def property_changed(prop, value, _, dbus_message):
            ...
            if prop == "org.freedesktop.NetworkManager.AccessPoint":
                if (
                    "Strength" in value
                    and dbus_message.path == self.wifi_access_point_path
                ):
            ...
            if (
                prop == "org.freedesktop.NetworkManager"
                and "ActiveConnections" in value
            ):
                self.run_coro(self.update_state())
    ...
    async def fetch_wifi_access_point(self):
        """
        Updates the currently connected access point path.
        """
...
class NetworkConnectionStatus(EventListener):
    ...
    async def update_state(self):
        """
        Triggers a state update with the network status
        """
